...
import com.example.TicketingSystem.dto.EditCommentDto;
...
public class TicketCommentsController {
  ...
  ticket.setUpdatedDate(LocalDateTime.now());
        ...
        notificationService.sendNotification(
                ...
                "You added a new comment on ticket " +ticket.getTicketId()
        );
...
//        );
     ...
     * @param editCommentDto The object containing the new comment text and user details.
    ...
    public ResponseEntity<?> editComment(
            @Parameter(description = "ID of the comment to be edited") @PathVariable String commentId,
            @RequestBody EditCommentDto editCommentDto
    ) {
        try {
            notificationService.sendNotification(editCommentDto.getUser(),
                    "You edited a comment on ticket "
            );

            return ResponseEntity.ok(ticketCommentService.editComment(
                    commentId,
                    editCommentDto.getNewComment(),
                    editCommentDto.getUser()
            ));
